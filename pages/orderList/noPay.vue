        ...
        <div class="content" v-if="orderData.length != 0">
            <div class="card" v-for="(item,value) in orderData" :key="value">
                ...
                <div id="shopItem" v-for="(second,value) in item.orderShopInfoList" :key="value" @click="goOrderInfo(second.id)">
                    <p><img src="../../assets/carImage/shopLogo.png" alt="">
                ...
                <div class="face" v-if="item.status == 5 || item.status == 1 || item.status == 6">
                    <span v-if="item.status == 5">实付￥ </span>
                    <span v-if="item.status == 5">{{item.payment}}</span>
                    <button @click="payWayDisplay(item.billNo,item.payment)">{{item.status == 1 ? '继续购买' : '再次购买'}}</button>
                </div>
...
export default {
    ...
    methods: {
        ...
        pay(item) {
            this.display = !this.display;
            //  this.$router.push({ path: "/pay/pay", query: { orderId: item.id } });
        },
        // 前往主页
        goIndex() {
        ...
        goPay() {
            if (this.way == 1) {
                var billNo = this.billNo;
                var key = localStorage.getItem("key");
                window.location.href = `http://api.ickapay.com/pay/payMall/alipay?billNo=${billNo}&key=${key}&openType=${0}`;
            }
        },
        //获取所有订单信息
        getData() {
            api.getStatusOrder(this.params).then(res => {
                if (!res.data) {
                    return;
                }
    ...
    mounted() {
        this.getData();
        api.getStatusOrderNum(1).then(res => {
            this.allOrderNum = res.data;
            if (res.data == 0) {
                this.show = true;
            }
        });
